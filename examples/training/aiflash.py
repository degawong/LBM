...
class AutoencoderKLDiffusers(BaseModel):
    ...
    def _get_properties(self):
        ...
        z = self.encode(x, sample_posterior=False)

        self.downsampling_factor = int(x.shape[2] / z.shape[2])

    def encode(self, x: torch.tensor, batch_size: int = 8, sample_posterior: bool = False):
        latents = []
        for i in range(0, x.shape[0], batch_size):
            posterior = self.vae_model.encode(x[i : i + batch_size]).latent_dist
            latent = posterior.mode() if not sample_posterior else posterior.sample()
