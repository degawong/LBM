...
class LBMModel(BaseModel):
    ...
    def __init__(
        ...
        prompt_path = (
            ...
            or getattr(config, "prompt_embedding_path", None)
            or os.environ.get("LBM_PROMPT_EMBEDDING_PATH")
            or "prompt_cache/6ab02325b7fc8d17.pt"
        )
        self.prompt_path = str(Path(prompt_path).expanduser())
        if not Path(self.prompt_path).is_file():
            raise FileNotFoundError(
                f"Prompt embedding not found: {self.prompt_path}. "
                "Set config.prompt_embedding_path or LBM_PROMPT_EMBEDDING_PATH."
            )
        ...
        print(f"[LBMModel] loading prompt embedding from: {self.prompt_path}")
        print(f"[LBMModel] prompt embedding shape: {tuple(self.prompt_embedding.shape)}")
    ...
    def forward(self, batch: Dict[str, Any], step=0, batch_idx=0, *args, **kwargs):
        ...
        if self.latent_loss_weight > 0:
            if self.mask_key in batch and valid_mask_for_latent.float().mean().item() == 1.0:
                loss = torch.mean(
                    ((prediction - target.detach()) ** 2).reshape(prediction.shape[0], -1),
                    1,
                )
            else:
